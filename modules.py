...
def gru(inputs, num_units=None, bidirection=False, scope="gru", reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        if bidirection: 
            ...
            outputs, _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(cell, cell_bw, inputs, dtype=tf.float32)
            return tf.concat(outputs, 2)  
        else:
            outputs, _ = tf.compat.v1.nn.dynamic_rnn(cell, inputs, dtype=tf.float32)
            return outputs
...
def attention_decoder(inputs, memory, num_units=None, scope="attention_decoder", reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        attention_mechanism = tfa.seq2seq.BahdanauAttention(num_units, 
                                                                   memory)
        decoder_cell = tf.compat.v1.nn.rnn_cell.GRUCell(num_units)
        cell_with_attention = tfa.seq2seq.AttentionWrapper(decoder_cell,
                                                                  attention_mechanism,
                                                                  num_units,
                                                                  alignment_history=True)
        outputs, state = tf.compat.v1.nn.dynamic_rnn(cell_with_attention, inputs, dtype=tf.float32) #( N, T', 16)

    return outputs, state

def prenet(inputs, num_units=None, is_training=True, scope="prenet", reuse=None):
    '''Prenet for transcript_encoder and Decoder1.
    Args:
