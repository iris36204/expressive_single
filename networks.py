...
def transcript_encoder(inputs, is_training=True, scope="encoder", reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse): 
        # Encoder pre-net
        prenet_out = prenet(inputs, is_training=is_training) # (N, Tx, E/2) shape=(32, 188, 128)
        ...
        ## Max pooling
        enc = tf.compat.v1.layers.max_pooling1d(enc, pool_size=2, strides=1, padding="same")  # (N, Tx, K*E/2)
...
def reference_encoder(inputs, is_training=True, scope="encoder", reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        tensor = tf.compat.v1.layers.conv2d(inputs=inputs, filters=32, kernel_size=3, strides=2, padding='SAME')
        tensor = bn(tensor, is_training=is_training, activation_fn=tf.nn.relu, scope="bn1")

        tensor = tf.compat.v1.layers.conv2d(inputs=tensor, filters=32, kernel_size=3, strides=2, padding='SAME')
        tensor = bn(tensor, is_training=is_training, activation_fn=tf.nn.relu, scope="bn2")

        tensor = tf.compat.v1.layers.conv2d(inputs=tensor, filters=64, kernel_size=3, strides=2, padding='SAME')
        ...
        tensor = tf.compat.v1.layers.conv2d(inputs=tensor, filters=128, kernel_size=3, strides=2, padding='SAME')
        ...
        # FC -> (N, 128)
        prosody = tf.compat.v1.layers.dense(tensor, 128, activation=tf.nn.tanh)
...
def decoder1(inputs, memory, is_training=True, scope="decoder1", reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        mel_hats = tf.compat.v1.layers.dense(dec, hp.n_mels*hp.r)
...
def decoder2(inputs, is_training=True, scope="decoder2", reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        # Max pooling
        dec = tf.compat.v1.layers.max_pooling1d(dec, pool_size=2, strides=1, padding="same") # (N, Ty, E*K/2)
        ...
        dec = tf.compat.v1.layers.dense(dec, hp.embed_size//2) # (N, Ty, E/2)         
        # Highway Nets
        for i in range(4):
        ...
        outputs = tf.compat.v1.layers.dense(dec, 1+hp.n_fft//2)
